---
title: "Wildco_DB_to_Standardized.Rmd"
author: "Liam Brennan"
date: "2024-02-14"
...
```{r setup test, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, results = FALSE)
```

```{r db_standard_intitialize}
### 0. Initialize ####
library(tidyr)
library(dplyr)
library(readr)
...
stations <- read_csv(paste0("raw_data/", proj_name, "/stations.csv"))
camera_checks <- read_csv(paste0("raw_data/", proj_name, "/camera_checks.csv"))
...
write.csv(deployment_short, paste0("raw_data/", proj_name, "/", proj_name, "_deployment_data.csv"), row.names = F)
...
# Export to .csv
write.csv(stations, paste0("raw_data/", proj_name, "/", proj_name, "_station_data.csv"), row.names = F)

### 3. Standardize identifications from database output ####

idents <- read.csv(paste0("raw_data/", proj_name, "/images_idents.csv"))
ud_dat <- idents
...
# Write to csv
write.csv(ud_dat, paste0("raw_data/", proj_name, "/", proj_name, "_detection_data.csv"), row.names = F)
```
