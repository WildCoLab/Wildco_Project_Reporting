---
title: "Standardised_Data_Exploration_Single_Site.Rmd"
...
## README FIRST ##
#Read and run this chunk of code line by line in the R Console (do not press knit) - there are some questions below which you will have to answer and some logic tests to complete. 
```{r single_site_set_up}
...
dat <- read.csv(paste0("raw_data/", proj_name, "/", proj_name, "_detection_data.csv"), header = T)
eff <- read.csv(paste0("raw_data/", proj_name, "/", "eff.csv"), header = T) # from the Effort.Rmd Script
sta <- read.csv(paste0("raw_data/", proj_name, "/", proj_name, "_station_data.csv"), header = T)
...
category <- params$category # Typical WildCo options: Feature.Type  Bait.Type
...
ind_sex_age_output <- params$ind_sex_age_output

# Do you want to create additional summary plots excluding human data?
plot_without_humans <- params$plot_without_humans
...
# Load Packages
list.of.packages <- c("leaflet", "dplyr", "viridis", "kriging", "corrplot", "lubridate", "kableExtra", "purrr", "tidyr", "hms", "ggplot2", "png", "grid")
...
m <- leaflet() %>%
  ...
  addLegend("bottomleft",
    colors = col.cat, labels = levels(sta[, category]),
    title = category,
...
png(paste0("figures/", proj_name, "/Basic_Deployment_Plot.png"), height = 1000, width = 1000)
...
plot(
  c(
    min(eff$Camera.Deployment.Begin.Date, na.rm = T),
    max(eff$Camera.Deployment.End.Date, na.rm = T)
  ),
  c(1, n.stat),
  las = 1, ylab = "", xlab = "", type = "n", yaxt = "n"
)
...
abline(v = as.numeric(strptime(seq(
  as.Date(paste0(substr(min(eff$Camera.Deployment.Begin.Date, na.rm = T), 1, 7), "-01")),
...
for (i in 1:length(plot.order))
{
  ...
  for (j in 1:nrow(tmp))
  {
    lines(
      c(
        tmp$Camera.Deployment.Begin.Date[j],
...
img <- readPNG(paste0("figures/", proj_name, "/Basic_Deployment_Plot.png"))
grid.raster(img)
```
...
png(paste0("figures/", proj_name, "/Detection_Barplot.png"), height = 1000, width = 1000)
...
par(mar = c(5, 16, 1, 1))
barplot(det.sum.total$n, names.arg = paste0(
  det.sum.total$Species,
  " (n =", det.sum.total$n, ")"
...
img <- readPNG(paste0("figures/", proj_name, "/Detection_Barplot.png"))
grid.raster(img)
```
...
```{r detection summary, echo=F, results = FALSE, echo = FALSE, output_vars = "none", fig.show = 'hide'}
# Make a separate plot for each 20 stations For each 20 stations
# To do this make a plot dataframe


# Generate species order and plot groups
plot.order <- sort(unique(eff$Deployment.Location.ID))
tmp4 <- data.frame("Deployment.Location.ID" = plot.order, "Plot.grp" = ceiling(1:length(unique(eff$Deployment.Location.ID)) / 20))

# Join the plot groups with the main data
eff <- left_join(eff, tmp4, by = "Deployment.Location.ID")

# Loop through each plot group and generate/save plots as separate PNG files
for (j in 1:length(unique(eff$Plot.grp))) {
  layout(matrix(c(1, 1, 1, 2), 1, 4, byrow = TRUE))
  par(mar = c(2, 6, 1, 1))

  plot(c(min(eff$Camera.Deployment.Begin.Date, na.rm = TRUE), max(eff$Camera.Deployment.End.Date, na.rm = TRUE)),
    c(1, length(unique(eff$Deployment.Location.ID[eff$Plot.grp == j]))),
    las = 1, ylab = "", xlab = "", type = "n", yaxt = "n"
  )

  axis(2,
    at = 1:length(unique(eff$Deployment.Location.ID[eff$Plot.grp == j])),
    labels = unique(eff$Deployment.Location.ID[eff$Plot.grp == j]), las = 1, cex.axis = 1
  )
  ...
  for (i in 1:length(unique(eff$Deployment.Location.ID[eff$Plot.grp == j]))) {
    ...
    for (k in 1:nrow(tmp)) {
      lines(c(tmp$Camera.Deployment.Begin.Date[k], tmp$Camera.Deployment.End.Date[k]), c(i, i), lwd = 2)
    }
  }
  ...
  png(paste0("figures/", proj_name, "/Detection_Check_Plot_Page_", j, ".png"), height = 1000, width = 1000)
...
for (j in 1:length(unique(tmp4$Plot.grp)))
{
  ...
  plot <- ggplot(tmp, aes(x = time_of_day, y = Species)) +
    geom_vline(
      xintercept = c(hms::as_hms("06:00:00"), hms::as_hms("18:00:00")),
    ...
    scale_x_time(
      name = "",
      breaks = c(
        hms::as_hms("00:00:00"), hms::as_hms("06:00:00"),
  ...
  ggsave(plot, filename = paste0("figures/", proj_name, "/Temporal_Activity_Plot_Page_", j, ".png"))
}
```
...
if (length(colnames(dat)[colnames(dat) == col.name] > 0)) {
  ...
  data_percentage <- apply(tmp, 2, function(x) {
    x * 100 / sum(x, na.rm = T)
  })
  plot <- TRUE
}
...
png(paste0("figures/", proj_name, "/Wildlife_Sex_Percent_Plot.png"), height = 1200, width = 800, res = 150)
...
img <- readPNG(paste0("figures/", proj_name, "/Wildlife_Sex_Percent_Plot.png"))
grid.raster(img)
```
...
png(paste0("figures/", proj_name, "/Age_Percent_Plot.png"), height = 1200, width = 800, res = 150)
...
img <- readPNG(paste0("figures/", proj_name, "/Age_Percent_Plot.png"))
grid.raster(img)
```
...
png(paste0("figures/", proj_name, "/Behaviour_Plot.png"), height = 1200, width = 800, res = 150)
...
img <- readPNG(paste0("figures/", proj_name, "/Behaviour_Plot.png"))
grid.raster(img)
```
...
if (!"Minimum.Group.Size" %in% colnames(dat)) {
...
top <- dat %>%
  group_by(Event.ID) %>%
  top_n(1, Date_Time.Captured) %>%
  select(Event.ID, Date_Time.Captured)
bot <- dat %>%
  group_by(Event.ID) %>%
  top_n(-1, Date_Time.Captured) %>%
  select(Event.ID, Date_Time.Captured)
...
diff$Date_Time.end <- NULL
diff$Date_Time.start <- NULL
dat$duration <- NULL
# Merge the data
dat <- dat %>%
...
write.csv(ind.dat, paste0("processed_data/", proj_name, "/", proj_name, "_", independent, "min_Independent.csv"), row.names = F)

# Also export the effort lookup
write.csv(row.lookup, paste0("processed_data/", proj_name, "/", proj_name, "_daily_effort_lookup.csv"), row.names = F)
```
...
if (ind_sex_age_output == TRUE) {
  ind.dat_sa <- dat_Event.ID_pre_subsetting %>%
    mutate(
      Sex = if_else(Sex == "", "Unknown", as.character(Sex)),
  ...
  # Save it for a rainy day
  write.csv(ind.dat_sa, paste0("processed_data/", proj_name, "/", proj_name, "_", independent, "min_Independent_by_sex_age.csv"), row.names = F)
}
```
...
png(paste0("figures/", proj_name, "/Independant_Detection_Plot.png"), height = 1000, width = 1000, res = 150)
...
img <- readPNG(paste0("figures/", proj_name, "/Independant_Detection_Plot.png"))
grid.raster(img)
```
...
png(paste0("figures/", proj_name, "/Independant_Detection_Plot_No_Humans.png"), height = 1200, width = 800, res = 150)
...
img <- readPNG(paste0("figures/", proj_name, "/Independant_Detection_Plot_No_Humans.png"))
grid.raster(img)
```
...
png(paste0("figures/", proj_name, "/Group_Size_Distribution_Plot.png"), height = 1200, width = 800, res = 150)
...
img <- readPNG(paste0("figures/", proj_name, "/Group_Size_Distribution_Plot.png"))
grid.raster(img)
```
...
png(paste0("figures/", proj_name, "/Covariance_Plot.png"), height = 2000, width = 2000, res = 150)
...
corrplot(M,
  method = "color", # col=matrix(col(200)),
...
img <- readPNG(paste0("figures/", proj_name, "/Covariance_Plot.png"))
grid.raster(img)
```
...
```{r all sites all species temporal plots, echo=F, eval=T}
...
png(paste0("figures/", proj_name, "/Summary_Temporal_Plot.png"), height = 1000, width = 1000, res = 150)
...
img <- readPNG(paste0("figures/", proj_name, "/Summary_Temporal_Plot.png"))
grid.raster(img)
```
...
```{r species trends, echo=FALSE, results='asis', eval=TRUE}
# Assuming mon.dat and focal.sp are your data and species lists

# Set the number of plots per page
plots_per_page <- 6

# Calculate the number of pages needed
num_pages <- ceiling(length(focal.sp) / plots_per_page)

# Loop through each page
for (page in 1:num_pages) {
  # Open a PNG device for each page
  png(paste0("figures/", proj_name, "/Species_Capture_Rate_Page_", page, ".png"), height = 1000, width = 1000)

  # Set up a 2x3 grid layout
  par(mfrow = c(2, 3))

  # Loop through each species on this page
  for (i in ((page - 1) * plots_per_page + 1):min(page * plots_per_page, length(focal.sp))) {
    # Ensure focal.sp is correctly referenced
    species_name <- as.character(focal.sp[i])
    print(paste("Plotting species:", species_name))

    plot(mon.dat$timestamp, mon.dat[, species_name],
      ylab = "Capture Rate per 100 days", xlab = "", type = "l", las = 1,
      main = species_name
    )

    points(mon.dat$timestamp, mon.dat[, species_name],
...
1. A data frame of "independent detections" at the `r independent` minute threshold you specified at the start: 
  ...
  - "`r  paste0("processed_data/",proj_name, "/", proj_name, "_",independent ,"min_Independent.csv")`"

2/3: A 'site x species' matrix of the number of independent detections and species counts across the full study period:

  - "`r  paste0("processed_data/",proj_name, "/", proj_name, "_",independent ,"min_Independent_total_observations.csv")`"

  - "`r  paste0("processed_data/",proj_name, "/", proj_name, "_",independent ,"min_Independent_total_counts.csv")`"
...
4/5: A 'site_month x species' matrix of the number of independent detections and species counts accross for each month in the study period:
  ...
  - "`r  paste0("processed_data/",proj_name, "/", proj_name, "_",independent ,"min_Monthly_total_observations.csv")`"

  - "`r  paste0("processed_data/",proj_name, "/", proj_name, "_",independent ,"min_Monthly_total_counts.csv")`"
...
6/7: A 'site_week x species' matrix of the number of independent detections and species counts accross for each week in the study period:
  ...
  - "`r  paste0("processed_data/",proj_name, "/", proj_name, "_",independent ,"min_Weekly_total_observations.csv")`"

  - "`r  paste0("processed_data/",proj_name, "/", proj_name, "_",independent ,"min_Weekly_total_counts.csv")`"
...
8/9: A 'site_day x species' matrix of the number of independent detections and species counts accross for each day a station was active in the study period:
  ...
  - "`r  paste0("processed_data/",proj_name, "/", proj_name, "_",independent ,"min_Daily_total_observations.csv")`"

  - "`r  paste0("processed_data/",proj_name, "/", proj_name, "_",independent ,"min_Daily_total_counts.csv")`"
...
```{r total counts, echo=F, message=F, warning=F}
# Total counts
...
write.csv(total.obs, paste0("processed_data/", proj_name, "/", proj_name, "_", independent, "min_Independent_total_observations.csv"), row.names = F)

write.csv(total.count, paste0("processed_data/", proj_name, "/", proj_name, "_", independent, "min_Independent_total_counts.csv"), row.names = F)
```

```{r monthly counts, echo=F, message=F, warning=F}
# Monthly counts
...
write.csv(mon.obs, paste0("processed_data/", proj_name, "/", proj_name, "_", independent, "min_Independent_Monthly_observations.csv"), row.names = F)

write.csv(mon.count, paste0("processed_data/", proj_name, "/", proj_name, "_", independent, "min_Independent_Monthly_counts.csv"), row.names = F)
```


```{r weekly counts, echo=F, message=F, warning=F}
# Weekly format
# Station / Month / Effort / Covariates / Species
tmp <- row.lookup
...
write.csv(week.obs, paste0("processed_data/", proj_name, "/", proj_name, "_", independent, "min_Independent_weekly_observations.csv"), row.names = F)

write.csv(week.count, paste0("processed_data/", proj_name, "/", proj_name, "_", independent, "min_Independent_weekly_counts.csv"), row.names = F)
```


```{r daily counts, echo=F, message=F, warning=F}
# Daily format
# Station / Month / Effort / Covariates / Species
tmp <- row.lookup
tmp$Effort <- 1
...
write.csv(day.obs, paste0("processed_data/", proj_name, "/", proj_name, "_", independent, "min_Independent_daily_observations.csv"), row.names = F)

write.csv(day.count, paste0("processed_data/", proj_name, "/", proj_name, "_", independent, "min_Independent_daily_counts.csv"), row.names = F)
```
...
### Observations
```{r observations, echo=F}
tmp <- cbind(
  data.frame("Time" = c("Total", "Monthly", "Weekly", "Daily")),
  rbind(
    colSums(total.obs[, 2:ncol(total.obs)]),
...
tmp <- cbind(
  data.frame("Time" = c("Total", "Monthly", "Weekly", "Daily")),
  rbind(
    colSums(total.count[, 2:ncol(total.count)]),
